  ...
  return (
    <div className="calendar-popover hud-frame" role="dialog" aria-label="Simulation calendar">
      ...
      <div className="calendar-grid">
        {cells.map((day, index) => <span className={day === date.getDate() ? "calendar-day current" : "calendar-day"} key={`${day}-${index}`}>{day || ""}</span>)}
      </div>
...
export default function TimeControls({ currentTime, playing, playbackRate, onTogglePlay, onSetRate, onStep }) {
  const [calendarOpen, setCalendarOpen] = useState(false);
  const rateMagnitude = Math.abs(playbackRate);
  const rateLabel = `${playbackRate < 0 ? "−" : "×"}${rateMagnitude}`;
  const dateKey = useMemo(() => currentTime.toISOString(), [currentTime]);
